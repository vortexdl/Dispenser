    ...
    try {
        ...
        if (subject === "Link Leaking" && devLinkLeakingChannelId) {
            try {
                ...
                const errorMsg = `Failed to send Link Leaking report to channel ${devLinkLeakingChannelId}`;
        ...
        if (subject === "Bot Issue" && devIssuesChannelId) {
            try {
                ...
                const errorMsg = `Failed to send Bot Issue report to dev channel ${devIssuesChannelId}`;
                logger.error(errorMsg, { error });
                return err(
                    new Error(
                        `${errorMsg}: ${
        ...
        const serverHasConfiguredChannel =
            guildConfig?.reportsChannelId && guildReportChannelId;
        if (subject === "Link Leaking") {
            ...
            confirmationMessage += ` and sent to the server's report channel (<#${guildConfig.reportsChannelId}>)`;
        } else if (
            subject !== "Bot Issue" &&
