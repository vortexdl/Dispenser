...
export async function autocompleteBannedUsers(
	...
	try {
		...
		for (const ban of bannedDocs) {
			try {
				...
				if (String(ban.userId).includes(lowerSearchValue)) {
					choices.push({
						name: `ID: ${ban.userId} (User data unavailable)`
							.substring(
								0,
								100,
							),
		...
		if (error instanceof MongoError || error instanceof MongoServerError) {
			const errorMsg =
				`Database error in autocompleteBannedUsers: ${error.message}`;
			logger.error(errorMsg, { error, guildId });
			return err(new Error(errorMsg));
		} else if (error instanceof Error) {
			const errorMsg =
				`Error in autocompleteBannedUsers: ${error.message}`;
			logger.error(errorMsg, { error, guildId });
			return err(new Error(errorMsg));
		} else {
			const errorMsg = `Unknown error in autocompleteBannedUsers: ${
				String(
					error,
				)
			}`;
...
export async function autocompleteNonBannedUsers(
	...
	try {
		...
		try {
			...
			const errorMsg =
				`Error fetching guild members for non-banned autocomplete: ${
		...
		if (error instanceof MongoError || error instanceof MongoServerError) {
			const errorMsg =
				`Database error in autocompleteNonBannedUsers: ${error.message}`;
			logger.error(errorMsg, { error, guildId });
			return err(new Error(errorMsg));
		} else if (error instanceof Error) {
			const errorMsg =
				`Error in autocompleteNonBannedUsers: ${error.message}`;
			logger.error(errorMsg, { error, guildId });
			return err(new Error(errorMsg));
		} else {
			const errorMsg = `Unknown error in autocompleteNonBannedUsers: ${
				String(
					error,
				)
			}`;
