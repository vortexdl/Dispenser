...
async function fetchData(
    ...
    try {
        ...
        if (dbErr instanceof MongoError || dbErr instanceof MongoServerError) {
    ...
    for (const item of linkDetails) {
        ...
        if (!basicExport) {
            ...
            if (item.addedTimestamp) {
                linkEntry
                    .ele("addedTimestamp")
                    .txt(item.addedTimestamp.toISOString());
            }
        }
    }
...
export async function handle(
    ...
    try {
        ...
        if (dbErr instanceof MongoError || dbErr instanceof MongoServerError) {
            logger.error(`A database${loggerMsgRest}: ${dbErr}`);
            await responder.editResponse(responseMsg);
            return;
        } else {
            logger.error(`An unexpected${loggerMsgRest}: ${dbErr}`);
            await responder.editResponse(responseMsg);
            return;
        }
    }
    ...
    if (format) {
        ...
        if (formattedData.length > maxLen) {
            codeBlockContent =
                formattedData.substring(0, maxLen) + "\n... (truncated)";
        }
        ...
        await responder.editResponseWithEmbedAndFiles(
            [embedToSend],
            [
                {
