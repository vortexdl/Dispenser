...
export async function handle(
	...
	if (
		!linkOption || typeof linkOption.value !== "string" ||
		!categoryOption || typeof categoryOption.value !== "string" ||
...
async function handleSingleLink(
	...
	const otherGuildInstances = await linksDb.find({
		link: link,
		guildId: { $ne: guildId },
	}).toArray();
	...
	if (otherGuildInstances.length > 0) {
		...
		const warningEmbed: Embed = {
			...
			description:
				`The link you're trying to add exists in ${otherGuildInstances.length} other server(s).\n\n**This may indicate link leaking.**\n\nDo you want to continue adding this link anyway?`,
			color: parseInt(guildConfig.theme.warning_color, 16),
			footer: {
				text:
					"If you continue, a notification will be sent to the other servers and bot developers",
			},
		};
		...
		const actionRow: ActionRow = {
			...
			components: [
				...
				},
				{
					...
					customId: `add_report_link_leaking_${
						btoa(link).substring(0, 80)
					}`,
				},
			] as any,
		};
...
async function generateResultsEmbed(
	...
	if (successful.length > 0) {
		...
		successful.forEach((result, index) => {
			if (index < 10) { // Limit display to first 10 to avoid embed limits
	...
	if (duplicates.length > 0) {
		description += `⚠️ **Already Exist (${duplicates.length}):**\n`;
		duplicates.forEach((result, index) => {
			if (index < 5) { // Limit display to first 5
				description += `• ${result.link}\n`;
			}
		});
	...
	if (leaks.length > 0) {
		...
		leaks.forEach((result, index) => {
			if (index < 5) { // Limit display to first 5
				description +=
					`• ${result.link} (found in ${result.leakGuilds} other server${
						result.leakGuilds === 1 ? "" : "s"
					})\n`;
			}
		});
		if (leaks.length > 5) {
	...
	if (errors.length > 0) {
		...
		errors.forEach((result, index) => {
			if (index < 3) { // Limit display to first 3
				description += `• ${result.link}: ${
	...
	return {
		...
		footer: {
			text:
				`${successful.length} added • ${duplicates.length} duplicates • ${leaks.length} leaks • ${errors.length} errors`,
		},
	};
}
...
async function sendLeakNotifications(
	...
	try {
		...
		const leakEmbed: Embed = {
			...
			description:
				`A link in your server was just attempted to be added to another server.\n\n**Server:** ${currentGuildName}\n**Link:** ${link}`,
			color: parseInt(guildConfig.theme.error_color, 16), // Use error_color for leak alert in other servers
			footer: {
				text:
					"This may indicate link leaking. Use the button below to report this to bot developers.",
			},
		};
			...
			components: [
				{
					...
					customId: `leak_report_link_leaking_${
						btoa(link).substring(0, 80)
					}`,
				},
			] as any,
...
async function sendDeveloperLeakNotification(
	...
	try {
		...
		for (let i = 0; i < otherInstances.length; i++) {
			...
			otherGuildsText += `${
				i + 1
			}. ${guildName} (ID: ${instance.guildId}) - Category: ${instance.cat}\n`;
		}
		...
		const devEmbed: Embed = {
			...
			description:
				`A link was attempted to be added to a server but was detected in other servers.\n\n**Adding Server:** ${currentGuildName}\n**Link:** ${link}\n\n**Existing in server(s):**\n${otherGuildsText}`,
		...
		await logger.debug("Sent the link leak notification to bot developers");
	} catch (error: unknown) {
